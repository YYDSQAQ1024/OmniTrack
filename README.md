...
声明
------
1. 穿透服务不是专门为JHH而设计的，主要是为内网穿透用户设计的。而且OmniTrack的服务器不在大陆，所以不会有备案的问题。
2. 当前插件仍处于测试阶段，难免会有BUG，因使用本插件导致的数据丢失需自行负责
3. 严禁将OmniTrack用于违法用途，否则后果自负
4. 使用OmniTrack提供的插件及服务即代表您同意此声明
